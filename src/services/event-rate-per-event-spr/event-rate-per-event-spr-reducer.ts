...
import { Filters, EventRatePerEventSpr, EventRatePerEventSprs, FilterNames, view, ChartData } from 'models';
import { EventRatePerEventSprsState } from '.';
...
// The initial state of the EventRatePerEventSpr page
export const initialState: EventRatePerEventSprsState = {
    eventRatePerEventSprs: [],
    eventRatePerEventSpr: undefined,
    filters: {},
    filterValues: {},
    categories: [],
    defaultValues: [],
 ...
 *
 */
const eventRatePerEventSprsSlice = createSlice({
    name: 'eventRatePerEventSprs',
    initialState,
    reducers: {
        getAllEventRatePerEventSprs(state, action: PayloadAction<Filters>) {
            state.filters = action.payload;
        },
        setAllEventRatePerEventSprs(state, action: PayloadAction<EventRatePerEventSprs>) {
            ...
            const { filterValues = {} as any } = transform(
                ...
                function (acc, value, key) {
                    ...
                    sortedValues.forEach((element) => {
                        const { date, eventCount = 0, rivetCount = 0, week } = element;
            ...
            state.eventRatePerEventSprs = series;
            state.defaultValues = series;
            state.categories = categories;
            state.filterValues = filterValues;
        },
        setEventRatePerEventSpr(state, action: PayloadAction<EventRatePerEventSpr>) {
            state.eventRatePerEventSpr = action.payload;
        },
        ...
        localFiltering(state, action: PayloadAction<Filters>) {
            ...
            state.eventRatePerEventSprs = filteredSeries;
        },
    },
});

export const {
    actions: eventRatePerEventSprActions,
    reducer: eventRatePerEventSprReducer,
    name: eventRatePerEventSprKey,
} = eventRatePerEventSprsSlice;
