...
export const TableHeader = memo((props: TableHeaderProps) => {
    ...
    const startOfMonth = moment(time).startOf('month').isoWeek();
    ...
    const renderSortLabel = (show: boolean, column: string, label: string) => {
        return (
            <Tooltip
                arrow
                title={<div className={'launch-tool-tip'}>{`Sort by ${label === 'TDT' ? 'Total DownTime (In Seconds)' : label}`}</div>}
    ...
    return (
        ...
        >
            ...
            {label !== 'cycleCount' && label !== 'ratio' && label !== 'rop' && label !== 'duration' && (
                <Div
