...
// import { faultRatesActions, faultRatesReducer, reportingViewSprKey } from 'services/fault/fault-rate/fault-rate-reducer';
...
import {
    reportingViewSprActions,
    reportingViewSprReducer,
    reportingViewSprKey,
} from 'services/reporting-view-spr/reporting-view-spr-reducer';

import {
    selectReportingViewSprs,
    selectReportingViewSprIsLoading,
    selectReportingViewSprFilterValues,
} from 'services/reporting-view-spr/reporting-view-spr-selectors';

import { getAllReportingViewSprsSaga } from 'services/reporting-view-spr/sagas/reporting-view-spr-saga-get-all';
...
import { ParentLabel } from './ParentLabel/index';
...
import { TableHeader as HourlyTableHeader, TableBody as HouryTableBody } from './components/hourly';
...
import { useQueryParam, getNumber, getDefaultFilterDate } from 'utils';
import FormControlLabel from '@mui/material/FormControlLabel';
import Switch from '@mui/material/Switch';
import { messages } from './messages';
import { useTheme } from '@mui/material/styles';
import './ReportingViewSpr.scss';
 ...
 * @returns
 */
export const ReportingViewSpr = memo((props: Props) => {
    useInjectReducer({ key: reportingViewSprKey, reducer: reportingViewSprReducer });
    useInjectSaga({ key: reportingViewSprKey, saga: getAllReportingViewSprsSaga });
    ...
    const defaultFilters = useMemo(
        () => ({
            ...
            [FilterNames.toTime]: START_TIME.clone()
                ...
                .endOf('isoWeek'),
            // [FilterNames.fromTime]: moment(2020-11-28+00:00:00),
            // [FilterNames.toTime]: '2022-02-19+23:59:59',
            ...
            [FilterNames.grouping]: 'deviceName',
    ...
    const faultRates: ReportingViewSprs | undefined = useSelector(selectReportingViewSprs);
    const filterValues = useSelector(selectReportingViewSprFilterValues);
    const faultRatesIsLoading = useSelector(selectReportingViewSprIsLoading);
    ...
    useEffect(() => {
        dispatch(
            reportingViewSprActions.getAllReportingViewSprs({
                ...apiFilters,
                ...breadCrumbsDataType,
            }),
        );
        ...
        (event, label, view: ReportingDataView, name?, time?: Moment | string | Moment) => {
            ...
            switch (view) {
                ...
                case ReportingDataView.Daily:
                    ...
                    if (moment(fromTime).isoWeek() >= moment().isoWeek()) {
    ...
    const getValue = (tableColumn: any, tData: any, showFaultRate: boolean): any => {
        ...
        tableColumn.forEach((item, index) => {
            ...
            if (index <= 0 || index === tableColumn.length - 1) {
        ...
        output.data = rows.map((row, i) => {
            ...
            const z = rowsData.map((row: any) =>
                row.reduce((a, b) => ({
                    ...
                    ripCount: +a?.ripCount + +b?.ripCount,
                    ropCount: +a?.ropCount + +b?.ropCount,
                    duration: +a?.duration + +b?.duration,
                })),
            );

            const { deviceName /*, studId*/ } = z[0];
            let finalData = [deviceName /*, studId*/]; // Commenting studId
            ...
            intervalRange.intervals.forEach((week: Moment, index: number) => {
                ...
                let { eventCount = 0, cycleCount = 0, ripCount = 0, ropCount = 0, eventRatio, ropRatio, duration = 0 } =
                    foundIt || ({} as any);
                eventCount = getNumber(+eventCount);
                cycleCount = getNumber(+cycleCount);
                ripCount = getNumber(+ripCount);
                ropCount = getNumber(+ropCount);
                duration = getNumber(+duration);

                let ratio = 0;
                let ripRopRatio = 0;
                if (!!foundIt) {
                    // Conditions to check if eventRatio and ropRatio are there or not which
                    // is provided by sigmoid API. If not calculate the values.
                    if (!eventRatio || !ropRatio) {
                        const denominator = eventCount + cycleCount;
                        const wr_denominator = ripCount + +ropCount;
                        ratio = denominator === 0 ? 0 : eventCount / denominator;
                        ripRopRatio = wr_denominator === 0 ? 0 : +ropCount / wr_denominator;
                    }
                    if (eventRatio) {
                        ratio = getNumber(+eventRatio);
                    }
                    if (ropRatio) {
                        ripRopRatio = getNumber(+ropRatio);
                    }
        ...
        if (intervalViews[reportingDataView].sortCompare) {
            dataColumnsOptions.sortCompare = (order) => (a, b) => {
                ...
                switch (sortColumn) {
                    ...
                    case 'rop':
                        const rop1 = a?.data?.ropCount,
                            rop2 = b?.data?.ropCount;
                        return (rop1 - rop2) * (order.direction === 'asc' ? 1 : -1);
                    case 'duration':
    ...
    const options: any = useMemo(() => {
        ...
        return {
            ...
            customTableBodyFooterRender: showFaultRate
                ? () => {}
                : function (opts) {
                      const cols = tableData.totals.slice(1) || [];
                      return (
                        ...
                        >
                            <Box
                                sx={{
                                    ...
                                    background: `${
                                        mode === 'light' ? palette?.grey?.[100] : '#243041 '
                                        ...
                                        >
                                            <Tooltip title="Table" placement="top-start">
                                                ...
                                                >
                                                    <i
                                                        ...
                                                        className="fa fa-table table-icon"
                                                        aria-hidden="true"
                                                    ></i>
                                                </Button>
                                            </Tooltip>
                                            <Tooltip title="Graph" placement="top-start">
                                                <Button
                                                    ...
                                                    // onClick={() => setReportingDataViewType(ReportingDataViewType.Graph)}
                                                >
                                                    <i className="fa fas fa-chart-line chart-icon"></i>
                                                </Button>
                                            </Tooltip>
                                        </ButtonGroup>
                                    )}
                        ...
                        >
                            ...
                            {reportingDataViewType === ReportingDataViewType.Table && tableData.data.length === 0 && (
                                <Box>
                                    ...
                                    {!faultRatesIsLoading && (
                                        <Box sx={{ textAlign: 'center' }}>{t(messages.noData)}</Box>
                                    )}
                                </Box>
                            )}
