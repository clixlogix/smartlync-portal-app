...
import { buildAvailableFiltersFromData, buildFiltersFromData, getDefaultFilterDate, useQueryParam } from 'utils';
import { useFilters } from 'utils/hooks/use-filters';
import { FaultsPerDeviceHistogramWidgetSprProperty, TopFaultCountSprProperty } from 'widgets';
import { FaultsPerDeviceHistogramSprWidget } from 'widgets/FaultsPerDeviceHistogramWidgetSpr/FaultsPerDeviceHistogramWidgetSpr';
import { TopFaultCountSpr } from 'widgets/TopFaultCountSpr/Loadable';
import { messages } from './messages';
import { Pages } from 'constants/defaultDateConfig';

interface TopXFaultCountAnalysisSprProps extends PageProps { }

const listOfLocalFilters = [];

export const TopXFaultCountAnalysisSpr: Page<TopXFaultCountAnalysisSprProps> = memo(
    (props: TopXFaultCountAnalysisSprProps) => {
        ...
        const pageFilters = useMemo(() => {
            ...
            return [
                ...
                ...(TopFaultCountSprProperty.defaultFilters || []),
                ...(FaultsPerDeviceHistogramWidgetSprProperty.defaultFilters || []),
            ];
        }, []);
        ...
        const topFaultCountGraphComponent = () => {
            return (
                <Box sx={{ height: '100%', width: '100%', display: 'flex', flexFlow: 'column nowrap', flex: 1 }}>
                    <FaultsPerDeviceHistogramSprWidget
                        onFilterChange={onFilterChange}
                        filters={filters}
                        localFilters={localFilters}
        ...
        return (
            <>
                <Helmet>
                    <title>{t(messages.topXFaultCountAnalysisSprPageTitle)}</title>

                    <meta name="description" content="Description of TopXFaultCountAnalysisSpr" />
                </Helmet>
                ...
                <Box sx={{ height: '100%', width: '100%', display: 'flex', flexFlow: 'column nowrap', flex: 1 }}>
                    ...
                    <Box sx={{ height: '100%', width: '100%', display: 'flex', flex: 1 }}>
