...
const Weekpicker = (props) => {
    ...
    const getDateRange = (date) => {
        ...
        let start = moment(defaultVal).subtract(8, 'isoWeek').startOf('isoWeek');
        let end = moment(defaultVal).endOf('isoWeek');

        if (!props.isNineWeekView) {
            start = moment(defaultVal).startOf('isoWeek');
            end = moment(defaultVal).endOf('isoWeek');
        }
    ...
    const formatWeekSelectLabel = (invalidLabel) => {
        ...
        if (props.isNineWeekView) {
            ...
            const dn = dateClone && dateClone.isValid() ? `[Week]${weeknumber} of ${year}` : invalidLabel;
            return dn;
        }
    };
    ...
    const RenderToolbar = (data) => {
        ...
        return (
            <MyToolbar
                ...
                isNineWeekView={props.isNineWeekView}
                start={start}
                date={selectedDate}
                end={end}
                {...props}
