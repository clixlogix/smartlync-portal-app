...
import {
    ...
    weldTimeSprReducer,
    weldTimeSprKey,
} from 'services/SPR/weld-time-spr/weld-time-spr-reducer';
import { selectWeldTimeSprs, selectWeldTimeSprIsLoading } from 'services/SPR/weld-time-spr/weld-time-spr-selectors';

import { getAllWeldTimeSprsSaga } from 'services/SPR/weld-time-spr/sagas/weld-time-spr-saga-get-all';
...
export const WeldTimeSprWidget: Widget<WeldTimeProps> = memo((props: WeldTimeProps) => {
    const { className = '', filters = {}, localFilters = {}, setAvailableFilters, onClick, getRef, refs } = props;

    useInjectReducer({ key: weldTimeSprKey, reducer: weldTimeSprReducer });
    useInjectSaga({ key: weldTimeSprKey, saga: getAllWeldTimeSprsSaga });

    const { t, i18n } = useTranslation();
    const weldTimes: WeldTimes | undefined = useSelector(selectWeldTimeSprs);
    const weldTimeIsLoading: boolean = useSelector(selectWeldTimeSprIsLoading);
    ...
    useEffect(() => {
        dispatch(weldTimeSprActions.getAllWeldTimeSprs(serviceFilters));
...
export const WeldTimeSprProperty = Object.assign(
    {},
    {
        defaultFilters,
        type: 'panel',
...
export default WeldTimeSprWidget;
