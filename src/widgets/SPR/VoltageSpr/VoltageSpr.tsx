...
import { voltageSprActions, voltageSprReducer, voltageSprKey } from 'services/SPR/voltage-spr/voltage-spr-reducer';
import { selectVoltageSprs, selectVoltageSprIsLoading } from 'services/SPR/voltage-spr/voltage-spr-selectors';

import { getAllVoltageSprsSaga } from 'services/SPR/voltage-spr/sagas/voltage-spr-saga-get-all';
...
import './VoltageSpr.scss';

interface VoltageSprProps extends ChartWidgetProps {
    getRef?(ref: ChartRef);
    refs?: ChartRef[];
    localFilters: Filters;
}

export const VoltageSprWidget: Widget<VoltageSprProps> = memo((props: VoltageSprProps) => {
    const { className = '', filters = {}, localFilters = {}, setAvailableFilters, onClick, getRef, refs } = props;

    useInjectReducer({ key: voltageSprKey, reducer: voltageSprReducer });
    useInjectSaga({ key: voltageSprKey, saga: getAllVoltageSprsSaga });

    const { t, i18n } = useTranslation();
    const voltages: Voltages | undefined = useSelector(selectVoltageSprs);
    const voltageIsLoading: boolean = useSelector(selectVoltageSprIsLoading);
    ...
    useEffect(() => {
        dispatch(voltageSprActions.getAllVoltageSprs(serviceFilters));
...
export const VoltageSprProperty = Object.assign(
    {},
    {
        defaultFilters,
        type: 'panel',
...
export default VoltageSprWidget;
