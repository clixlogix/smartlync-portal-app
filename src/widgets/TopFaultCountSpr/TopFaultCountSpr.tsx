...
import { getAllTopFaultCountSprsSaga } from 'services/top-fault-count-spr/sagas/top-fault-count-spr-saga-get-all';
import {
    topFaultCountSprActions,
    topFaultCountSprKey,
    topFaultCountSprReducer,
} from 'services/top-fault-count-spr/top-fault-count-spr-reducer';
import {
    selectTopFaultCountSprIsLoading,
    selectTopFaultCountSprs,
    selectTopFaultCountsSprColumns,
} from 'services/top-fault-count-spr/top-fault-count-spr-selectors';
...
// import './TopFaultCountSpr.scss';

interface TopFaultCountSprProps extends WidgetProps {
...
export const TopFaultCountSpr: React.FC<TopFaultCountSprProps> = memo((props: TopFaultCountSprProps) => {
    const { filters = {} } = props;
    useInjectReducer({ key: topFaultCountSprKey, reducer: topFaultCountSprReducer });

    useInjectSaga({ key: topFaultCountSprKey, saga: getAllTopFaultCountSprsSaga });

    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    const { t, i18n } = useTranslation();

    const topFaultCountSprs: any = useSelector(selectTopFaultCountSprs);
    const topFaultCountSprIsLoading: boolean = useSelector(selectTopFaultCountSprIsLoading);
    ...
    if (props.isLoading) {
        props.isLoading(topFaultCountSprIsLoading);
    }
    ...
    useEffect(() => {
        dispatch(topFaultCountSprActions.getAllTopFaultCountSprs(serviceFilters));
    }, [dispatch, serviceFilters]);

    const extraColumns: any = useSelector(selectTopFaultCountsSprColumns);
    ...
    return extraColumns.length > 0 ? (
        ...
        <>
            {topFaultCountSprIsLoading && <Loader circle />}
            {!topFaultCountSprIsLoading && <Loader noData />}
        </>
    );
});
...
export const TopFaultCountSprProperty = Object.assign(
    {},
    {
