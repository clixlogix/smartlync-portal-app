...
export function TableHeader(props: Props) {
    ...
    const renderSortLabel = (show: boolean, column: string, label: string, key = `${Math.random()}`) => {
        return (
            <Tooltip
                arrow
            ...
            {expand && (
                <Div className="value-labels stanleyRow" style={{ width: 180 }}>
                    {Columns.map(({ column, label }, index) =>
                        renderSortLabel(showSortIcon && sortColumn === column, column, label, `key-sort-col-${index}`),
                    )}
                </Div>
            )}
        </Th>
