...
import { FilterNames, Filters, Lifts } from 'models';
...
interface LiftProps extends ChartWidgetProps {
    onClick?(event);
    getRef?(ref: ChartRef);
    refs?: ChartRef[];
...
export const Lift: Widget<LiftProps> = memo((props: LiftProps) => {
    const { className = '', filters = {}, localFilters = {}, onClick, getRef, refs } = props;

    useInjectReducer({ key: liftKey, reducer: liftReducer });
    useInjectSaga({ key: liftKey, saga: getAllLiftsSaga });

    const { t, i18n } = useTranslation();
    const lift: Lifts | undefined = useSelector(selectLifts);

    const liftIsLoading: boolean = useSelector(selectLiftIsLoading);
    ...
    useEffect(() => {
        dispatch(liftActions.getAllLifts(serviceFilters));
...
export const LiftProperty = Object.assign(
    {},
    {
        defaultFilters,
        type: 'panel',
